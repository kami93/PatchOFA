...
@register_model("masked_ofa")
class MaskedOFAModel(TransformerModel):
    ...
    @staticmethod
    def add_args(parser):
        super(MaskedOFAModel, MaskedOFAModel).add_args(parser)
        parser.add_argument(
            "--pooler-dropout",
            type=float,
            metavar="D",
    ...
    def forward(
        ...
        original_x = img_masked_x = text_masked_x = None
        original_extra = img_masked_extra = text_masked_extra = None
        loss_names = loss_type.split('_')
        for loss_name in loss_names:
            if loss_name == 'original':
                original_x, original_extra = self.decoder(
                    prev_output_tokens,
            ...
            elif loss_name == 'imgmask':
                encoder_padding_mask_for_img_masking = encoder_padding_mask.clone()
                encoder_padding_mask_for_img_masking[:, :900] = True

                encoder_out['encoder_padding_mask'][0] = encoder_padding_mask_for_img_masking
                img_masked_x, img_masked_extra = self.decoder(
                    prev_output_tokens,
                    code_masks=code_masks,
                    encoder_out=encoder_out,
                    features_only=features_only,
            ...
            elif loss_name == 'textmask':
                encoder_padding_mask_for_text_masking = encoder_padding_mask.clone()
                encoder_padding_mask_for_text_masking[:, 900:] = True
                encoder_out['encoder_padding_mask'][0] = encoder_padding_mask_for_text_masking
                text_masked_x, text_masked_extra = self.decoder(
                    prev_output_tokens,
                    code_masks=code_masks,
                    encoder_out=encoder_out,
                    features_only=features_only,
                ...
                raise ValueError(f"flossname {loss_name} not supported")
        ...
        return [original_x, img_masked_x, text_masked_x], [original_extra, img_masked_extra, text_masked_extra]
...
@register_model_architecture("masked_ofa", "masked_ofa_large")
def masked_ofa_large_architecture(args):
...
@register_model_architecture("masked_ofa", "masked_ofa_base")
def masked_ofa_base_architecture(args):
    ...
    masked_ofa_large_architecture(args)


@register_model_architecture("masked_ofa", "masked_ofa_huge")
def masked_ofa_huge_architecture(args):
...
@register_model_architecture("masked_ofa", "masked_ofa_medium")
def masked_ofa_medium_architecture(args):
...
@register_model_architecture("masked_ofa", "masked_ofa_tiny")
def masked_ofa_tiny_architecture(args):

...
class SelfPatchHead(nn.Module):
    ...
    def forward(self, img, word, word_batch_idx):
        bsz = img.size(0)
        ...
        if word is not None:
            ...
            img_norm_repeat = img_norm[word_batch_idx]

            # sim_matrix = word_norm @ img_norm.transpose(-2,-1)
            sim_matrix = torch.einsum("wd,wpd->wp", word_norm, img_norm_repeat)
            ...
            glo_tokens = self.norm(glo_tokens)
            loc_tokens = self.norm(loc_tokens)

...
@register_criterion(
    "seg_criterion_mlp", dataclass=SegCriterionMLPConfig
)
class SegCriterionMLP(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def get_lprobs_and_target(self, model, net_output, sample, return_logit=False):
        ...
        # target_lowres = target_lowres.reshape(bsz, seq_len)
        
        # bsz, seq_len, embed_dim = logits.size()
        # target = target.reshape(bsz, seq_len)
        ...
        #     # logits_lowres = logits_lowres[~eos_indices].reshape(bsz, seq_len, embed_dim)
        #     target_lowres = target_lowres[~eos_indices].reshape(bsz, seq_len)

        #     bsz, seq_len, embed_dim = logits.size()
        #     eos_indices = target.eq(self.task.tgt_dict.eos())
        #     # logits = logits[~eos_indices].reshape(bsz, seq_len, embed_dim)
        #     target = target[~eos_indices].reshape(bsz, seq_len)
        ...
        return lprobs.reshape(-1, lprobs.size(-1)), target.reshape(-1), lprobs_lowres.reshape(-1, lprobs_lowres.size(-1)), target_lowres.reshape(-1), constraint_masks

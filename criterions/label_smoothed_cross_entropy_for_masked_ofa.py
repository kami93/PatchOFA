...
@dataclass
class AdjustLabelSmoothedCrossEntropyCriterionConfigMaskedOFA(FairseqDataclass):
    ...
    loss_type: str = field(
        default='original_imgmask_textmask', metadata={"help": "loss_type."}
    )
...
@register_criterion(
    "adjust_label_smoothed_cross_entropy_masked_ofa", dataclass=AdjustLabelSmoothedCrossEntropyCriterionConfigMaskedOFA
)
class AdjustLabelSmoothedCrossEntropyCriterionMaskedOFA(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
        ...
        loss_type='original_imgmask_textmask'
    ):
        super().__init__(task)
    ...
    def forward(self, model, sample, update_num=0, reduce=True):
        ...
        aux_input = {"loss_type": self.loss_type}

        [original_x, img_masked_x, text_masked_x], [original_extra, img_masked_extra, text_masked_extra] = model(**net_input, **aux_input)
        ...
        logging_output = {
            "loss": loss.data,
            "original_loss": original_loss.data,
            "img_masked_loss": img_masked_loss.data,
            "text_masked_loss": text_masked_loss.data,
            "nll_loss": nll_loss.data,
            "original_nll_loss": original_nll_loss.data,
            "img_masked_nll_loss": img_masked_nll_loss.data,
            "text_masked_nll_loss": text_masked_nll_loss.data,
            "ntokens": sample["ntokens"],
            "nsentences": sample["nsentences"],
            "sample_size": sample_size,
        }
        if self.report_accuracy:
            n_correct, total = self.compute_accuracy(model, (original_x, original_extra), sample)
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        original_loss_sum = sum(log.get("original_loss", 0) for log in logging_outputs)
        img_masked_loss_sum = sum(log.get("img_masked_loss", 0) for log in logging_outputs)
        text_masked_loss_sum = sum(log.get("text_masked_loss", 0) for log in logging_outputs)

        original_nll_loss_sum = sum(log.get("original_nll_loss", 0) for log in logging_outputs)
        img_masked_nll_loss_sum = sum(log.get("img_masked_nll_loss", 0) for log in logging_outputs)
        text_masked_nll_loss_sum = sum(log.get("text_masked_nll_loss", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "original_loss", original_loss_sum / sample_size, sample_size, round=3
        )
        metrics.log_scalar(
            "img_masked_loss", img_masked_loss_sum / sample_size, sample_size, round=3
        )
        metrics.log_scalar(
            "text_masked_loss", text_masked_loss_sum / sample_size, sample_size, round=3
        )
        metrics.log_scalar(
            "nll_loss", nll_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_scalar(
            "original_nll_loss", original_nll_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_scalar(
            "img_masked_nll_loss", img_masked_nll_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_scalar(
            "text_masked_nll_loss", text_masked_nll_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_derived(

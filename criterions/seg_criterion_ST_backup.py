...
class SegCriterion(FairseqCriterion):
    def __init__(
        ...
        self.output_classes = 150 #task.cfg.num_seg_tokens
    ...
    def forward(self, model, sample, update_num=0, reduce=True, ema_model=None):
        ...
        if self.unsupervised_segmentation and model.training:
            ...
            with torch.inference_mode():
                ema_output = ema_model(**sample["net_input"], full_context_alignment=self.full_context_alignment)

            ema_logits, extra = ema_output
            ema_preds = self.upsample_logits(ema_logits)
            ema_prob = (ema_preds).softmax(-1)
            target = sample.get("target") - self.seg_id_offset
            for i in range(171):
                seg_masks = target.eq(i)
                target[seg_masks] = ema_prob[seg_masks].mean(dim=-2).argmax(-1) + self.seg_id_offset
            unknown_masks = target.eq(171)
            target[unknown_masks] = self.seg_id_offset + self.output_classes
            padding_masks = target.eq(self.padding_idx - self.seg_id_offset)
            target[padding_masks] = self.padding_idx
            eos_masks = target.eq(self.task.tgt_dict.eos() - self.seg_id_offset)
            target[eos_masks] = self.task.tgt_dict.eos()
            ...
            unknown_masks = target == 171
            target[unknown_masks] = self.output_classes
            sample["ori_semantic_seg"][0] = target

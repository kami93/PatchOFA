...
@dataclass
class SegCriterionV4Config(FairseqDataclass):
...
@register_criterion(
    "seg_criterion_v4", dataclass=SegCriterionV4Config
)
class SegCriterionV4(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, update_num=0, reduce=True):
        ...
        if self.alpha != 1.0:
            net_output = model(**sample["net_input"], full_context_alignment=self.full_context_alignment, aux_input=sample["aux_input"], net_input_aug=sample["net_input_aug"])
            
            labeled_output = net_output[1].get("aux_output")
            aug_output = net_output[1].get("aug_output")
        ...
        seg_loss, nll_loss, threshold_mask, ntokens, area_intersect_lowres, area_pred_label_lowres, area_label_lowres, area_union_lowres, area_intersect, area_pred_label, area_label, area_union = compute_seg_loss(model, net_output, aug_output, sample, update_num, reduce=reduce)
    ...
    def reshape_logits_and_target(self, model, logits, sample, alt_logits=None):
        ...
        target_lowres = sample["downsampled_target"].clone()
    ...
    def compute_unlabled_kld_loss(self, model, net_output, aug_output, sample, update_num, reduce=True):
        classifier_logits, extra = net_output
        
        if aug_output is not None:
            aug_classifier_logits, aug_extra = aug_output

        aug_logits = aug_logits_lowres = None
        if self.unlabeled_head_type == 'shared':
            ...
            if aug_output is not None:
                [aug_logits, aug_logits_lowres], _, _ = self.reshape_logits_and_target(model, aug_classifier_logits, sample)
            ...
            if aug_output is not None:
                aug_features = aug_extra.get('penultimate')
                aug_separate_logits = self.unlabeled_head(aug_features)
                [aug_logits, aug_logits_lowres], _, _ = self.reshape_logits_and_target(model, aug_separate_logits, sample)

        assert self.ignore_eos
        
        unlabeled_loss = torch.tensor([0.0], device=classifier_logits.device)
        threshold_mask = torch.tensor([1.0], device=classifier_logits.device)
        ntokens = 1
        if aug_output is not None:
            if self.upscale_lprobs:
                logits_train = logits
                logits_teacher = aug_logits
                
                target_train = target
                
            else:
                logits_train = logits_lowres
                logits_teacher = aug_logits_lowres

...
@dataclass
class CustomCriterionV4_2Config(FairseqDataclass):
    ...
    student_temperature: float = field(
        default=0.1, metadata={"help": "student_temperature for semisup loss"}
    )
    teacher_temperature: float = field(
        default=0.1, metadata={"help": "teacher_temperature for semisup loss"}
    )
    alpha: float = field(
        default=0.5, metadata={"help": "alpha weight"}
    )
    teacher_checkpoint_file: str = field(
        default='', metadata={"help": "teacher_checkpoint_file"}
    )
...
@register_criterion(
    "custom_criterion_v4-2", dataclass=CustomCriterionV4_2Config
)
class CustomCriterionV4_2(FairseqCriterion):
    ...
    def __init__(
        ...
        self.teacher_checkpoint_file = teacher_checkpoint_file
        if not osp.exists(teacher_checkpoint_file):
            raise ValueError(f"teacher_checkpoint_file {teacher_checkpoint_file} does not exist!")

        self.teacher_token_ln = LayerNorm(embed_dim)
        self.teacher_token_head = HeadMlp(in_features=embed_dim, hidden_features=embed_dim, out_features=dictionary_size, nlayers=self.token_head_depth)
        for param in self.teacher_token_ln.parameters():
            param.requires_grad_(False)
        for param in self.teacher_token_head.parameters():
            param.requires_grad_(False)
        ...
        logger.info(f"Teacher Temperature: {self.teacher_temperature}")
        logger.info(f"Student Temperature: {self.student_temperature}")
    ...
    def forward(self, model, sample, update_num=0, reduce=True):
        ...
        if self.teacher_encoder is None:
            self.teacher_encoder = copy.deepcopy(model.encoder)
            teacher_ckpt = torch.load(self.teacher_checkpoint_file, map_location='cpu')
            logger.info("lazy initializing teacher layers...")

            encoder_state_dict = dict()
            for key, value in teacher_ckpt['model'].items():
                if key.startswith('encoder.'):
                    encoder_state_dict[key[8:]] = value

            missing_keys, unexpected_keys = self.teacher_encoder.load_state_dict(encoder_state_dict, strict=False)
            if len(missing_keys):
                for name in missing_keys:
                    logger.info(f"Missing a teacher_encoder parameter: {name}")
            if len(unexpected_keys):
                for name in missing_keys:
                    logger.info(f"Unexpected a teacher_encoder parameter: {name}")

            for param in self.teacher_encoder.parameters():
                param.requires_grad_(False)

            token_ln_state_dict = dict()
            for key, value in teacher_ckpt['criterion'].items():
                if key.startswith('token_ln.'):
                    token_ln_state_dict[key[9:]] = value

            missing_keys, unexpected_keys = self.teacher_token_ln.load_state_dict(token_ln_state_dict, strict=False)
            if len(missing_keys):
                for name in missing_keys:
                    logger.info(f"Missing a teacher_token_ln parameter: {name}")
            if len(unexpected_keys):
                for name in missing_keys:
                    logger.info(f"Unexpected a teacher_token_ln parameter: {name}")

            for param in self.teacher_token_ln.parameters():
                param.requires_grad_(False)

            token_head_state_dict = dict()
            for key, value in teacher_ckpt['criterion'].items():
                if key.startswith('token_head.'):
                    token_head_state_dict[key[11:]] = value

            missing_keys, unexpected_keys = self.teacher_token_head.load_state_dict(token_head_state_dict, strict=False)
            if len(missing_keys):
                for name in missing_keys:
                    logger.info(f"Missing a teacher_token_head parameter: {name}")
            if len(unexpected_keys):
                for name in missing_keys:
                    logger.info(f"Unexpected a teacher_token_head parameter: {name}")
        ...
        loss, nll_loss, labled_loss, unlabled_loss, ntokens, batch_size = self.compute_loss(model, net_output, sample, update_num, reduce=reduce)
        ...
        unlabled_loss = unlabled_loss * sample_size
        semisup_loss = (1.0 - self.alpha) * labled_loss + self.alpha * unlabled_loss
        ...
        logging_output = {
            ...
            "unlabled_loss": unlabled_loss.data,
    ...
    def compute_loss(self, model, net_output, sample, update_num, reduce=True):
        ...
        with torch.no_grad():
            teacher_encoder_returns = self.teacher_encoder(
                net_input['src_tokens'],
                src_lengths=net_input['src_lengths'],
                patch_images=net_input['patch_images'],
                patch_masks=net_input['patch_masks'],
                patch_images_2=None,
            ...
            teacher_encoder_out = teacher_encoder_returns['encoder_out'][0]
            teacher_img_encoding = teacher_encoder_out[:900] # 'T B D'
            teacher_v_word_encoding = teacher_img_encoding.transpose(0, 1).reshape(B*900, -1)
            teacher_v_word_logit = self.teacher_token_head(self.teacher_token_ln(teacher_v_word_encoding))

        if self.loss_type == 'cross_entropy':
            with torch.no_grad():
                v_word_pred = F.softmax(teacher_v_word_logit / self.teacher_temperature, dim=-1)
                pred_max = v_word_pred.max(1)
                mask = (pred_max[0] >= self.unlabeled_threshold)
                targets_pseudo = pred_max[1]

            unlabled_loss = (F.cross_entropy(v_word_logit / self.student_temperature, targets_pseudo.detach(), reduction='none') * mask).mean()
            ...
            unlabled_loss = -torch.mean(torch.sum(F.log_softmax(v_word_logit / self.student_temperature, dim=1) * v_word_pred.detach() , dim=1) * mask)
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        unlabled_loss_sum = sum(log.get("unlabled_loss", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "unlabled_loss", unlabled_loss_sum / sample_size, ntokens, round=3
        )
        metrics.log_scalar(

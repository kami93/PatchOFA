...
def shot_noise(x, severity=1, rng=None):
    ...
    x = np.array(x) / 255.
    return np.clip(rng.poisson(x * c) / float(c), 0, 1) * 255
...
def impulse_noise(x, severity=1, rng=None):
    ...
    x = sk.util.random_noise(np.array(x) / 255., mode='s&p', seed=rng, amount=c)
...
def speckle_noise(x, severity=1, rng=None):
    ...
    x = np.array(x) / 255.
    return np.clip(x + x * rng.normal(size=x.shape, scale=c), 0, 1) * 255
...
def glass_blur(x, severity=1, rng=None):
    if rng is None:
        rng = np.random.default_rng()
    H, W = x.size
...
def fog(x, severity=1, rng=None):
    ...
    mapsize = 2**math.ceil(math.log2(H * 256 / 224)) # mapsize w.r.t 224
...
def frost(x, severity=1, rng=None):
    ...
    pil_frost = Image.fromarray(frost)
    FH, HW = frost.shape[:2]

    pil_frost = pil_frost.resize(size=(int(FH * H / 224), int(HW * W / 224))) # upscale frost with respect to 224
    frost = np.asarray(pil_frost)

    # randomly crop and convert to rgb
    x_start, y_start = rng.integers(0, frost.shape[0] - H), rng.integers(0, frost.shape[1] - W)
...
def snow(x, severity=1, rng=None):
    ...
    snow_layer = rng.normal(size=x.shape[:2], loc=c[0], scale=c[1])  # [:2] for monochrome
...
def jpeg_compression(x, severity=1, rng=None):
...
def elastic_transform(image, severity=1, rng=None):
    if rng is None:
        rng = np.random.default_rng()
    ...
    c = [(H * 2, H * 0.7, H * 0.1),
         ...
         (H * 0.12, H * 0.01, H * 0.02)][severity - 1]
    ...
    dx = (gaussian(rng.uniform(-1, 1, size=shape[:2]),
                   c[1], mode='reflect', truncate=3) * c[0]).astype(np.float32)
    dy = (gaussian(rng.uniform(-1, 1, size=shape[:2]),
                   c[1], mode='reflect', truncate=3) * c[0]).astype(np.float32)
